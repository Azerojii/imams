...
export default async function QuranTeachersPage({
  ...
  const paginatedTeachers = filtered.slice(startIndex, startIndex + PAGE_SIZE)
        ...
        <main className="px-4 py-4 md:px-6">
          ...
          {filtered.length === 0 ? (
            <div className="py-16 text-center text-text-secondary">
              ...
              {selectedWilaya && (
                <Link href="/quran-teachers" className="mt-2 inline-block text-primary hover:underline">
                  عرض الكل
                </Link>
              )}
            </div>
